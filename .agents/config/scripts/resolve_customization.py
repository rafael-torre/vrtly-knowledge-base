#!/usr/bin/env python3
"""
Resolve customization for a DB90 skill using three-layer TOML merge.

Reads customization from three layers (highest priority first):
  1. {project-root}/.agents/config/custom/{name}.user.toml  (personal, gitignored)
  2. {project-root}/.agents/config/custom/{name}.toml        (team/org, committed)
...
def find_project_root(start: Path):
    ...
    while True:
        if (current / ".agents").exists() or (current / ".git").exists():
            return current
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve customization for a DB90 skill using three-layer TOML merge.",
        add_help=True,
    )
    parser.add_argument(
    ...
    # an ancestor of cwd happens to have a stray .agents/ from another project.
    project_root = find_project_root(skill_dir) or find_project_root(Path.cwd())

    team = {}
    user = {}
    if project_root:
        custom_dir = project_root / ".agents" / "config" / "custom"
