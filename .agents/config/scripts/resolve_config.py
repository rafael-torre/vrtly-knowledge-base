#!/usr/bin/env python3
"""
Resolve DB90's central config using four-layer TOML merge.

Reads from four layers (highest priority last):
  1. {project-root}/.agents/config/config.toml              (base team)
  2. {project-root}/.agents/config/config.user.toml         (base user)
  3. {project-root}/.agents/config/custom/config.toml       (human-authored team, committed)
  4. {project-root}/.agents/config/custom/config.user.toml  (human-authored user, gitignored)
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve DB90 central config using four-layer TOML merge.",
    )
    parser.add_argument(
        "--project-root", "-p", required=True,
        help="Absolute path to the project root (contains .agents/config/)",
    )
    parser.add_argument(
    ...
    base_team = load_toml(config_dir / "config.toml", required=True)
    base_user = load_toml(config_dir / "config.user.toml")
    custom_team = load_toml(config_dir / "custom" / "config.toml")
    custom_user = load_toml(config_dir / "custom" / "config.user.toml")
