...
```bash
python3 {project-root}/.agents/config/scripts/resolve_config.py --project-root {project-root} --key agents
```

The resolver merges four layers in order: `.agents/config/config.toml` (base, team-scoped), `.agents/config/config.user.toml` (base, user-scoped), `.agents/config/custom/config.toml` (team overrides), and `.agents/config/custom/config.user.toml` (personal overrides). Each entry under `agents` is keyed by the agent's `code` and carries `name`, `title`, `icon`, `description`, `module`, and `team`.
